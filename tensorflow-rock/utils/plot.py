...
class PlotContext:
    ...
    def append(self, image_numpy, label):
        if self.__index + 1 > self.__ncols * self.__nrows:
            print("WARN:Your set nrow={0},ncols={1},but set {2} image".format(self.__nrows, self.__ncols, self.__index))
        else:
            plt.subplot(self.__nrows, self.__ncols, self.__index + 1)
            plt.xticks([])
            plt.yticks([])
            plt.grid(False)
            plt.imshow(image_numpy, cmap=plt.cm.binary)
            plt.xlabel(label)
            self.__index += 1
